...
export class Dashboard {
	...
	protected getFee(team: Team): void {
		...
		this.server.get(["team", team.id, "season", this.fee_season_id, "fee"].join("/")).subscribe((feeData: any) => {
			...
			{ text: 'Pokyny k zaplacení členských příspěvků ' + ' ' + this.server.getType("season", this.fee_season_id, "name"), style: 'header' },
			...
			{ text: "Hráči, kteří jsou registrování ve vašem oddíle, ale hráli také za jiný oddíl", style: "subHeader" },
			{
				table: {
					headerRows: 1,
