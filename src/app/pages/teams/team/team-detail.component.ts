...
export class TeamDetailComponent implements OnInit {
  ...
  public addressFormShowed = false;
  ...
  pardonFee(): void {
    ...
    this.server.get(['player', this.userForm.value.id, "history"].join("/")).subscribe(val => {
      ...
      if (canPardon) {
        this.server.post('admin/fee/pardon', {
          player_id: this.userForm.value.id,
          season_id: this.userForm.value.pardonFee
        }).subscribe(val => {
  ...
  public openUserForm(event, create?: boolean) {
    ...
    if (!create) {
      ...
      this.userForm.get("gdpr").setValue(event.data.gdpr);
      this.userForm.get("pardonFee").setValue(this.server.getType("season")[0].id);
    } else {
      this.userForm.get("team").setValue(this.team.id);
    }

    this.server.get('player/' + event.data.id + '/address').subscribe(addresses => {
  ...
  showAddressForm(data): void {
    this.addressFormShowed = true;
  }
